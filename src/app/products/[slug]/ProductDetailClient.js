...
// `product`/`prevProduct`/`nextProduct` arrive already resolved server-side
// (see page.js) — no more parsing the slug out of usePathname client-side.
const ProductDetailClient = ({ product, prevProduct, nextProduct }) => {
  const { t } = useLanguage();
      ...
      <section className="cta py-24 bg-gradient-to-r from-slate-900 to-slate-800 text-center text-white">
        <div className="container mx-auto px-6">
          ...
          <p className="text-xl text-slate-300 mb-10 max-w-2xl mx-auto">
            Kontakt oss i dag for å integrere våre intelligente, skalerbare SaaS-løsninger i din arbeidsflyt.
          </p>
          <Link
            href="/free-consultation"
