...
const TestimonialCard = ({ text, imgSrc, imgAlt, client }) => {
  return (
    <div className="relative group transition-all duration-500 hover:-translate-y-2 rounded-3xl z-0 h-full">
      
      {/* The glowing border container (expands 3px outward to form the thick border line) */}
      <div className="absolute -inset-[3px] rounded-[27px] overflow-hidden bg-slate-200 dark:bg-slate-800 transition-opacity duration-500 pointer-events-none z-0 opacity-0 group-hover:opacity-100">
      ...
      <div className="glass relative rounded-3xl p-8 hover:shadow-2xl border border-white/5 h-full z-10 bg-white dark:bg-slate-900 overflow-hidden">
